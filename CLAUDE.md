...
- Other live workflows: Topic Suggestor, Daily Briefing, PostHog Monitor, Notion Publish Status (webhook on PR merge: Notion + Slack + per-post IndexNow; Google Indexing API branch built but OFF), Error Trigger. **PR + Backlink Monitor is OFF** (deactivated 2026-07-14 at Ian's direction after 12 days live produced zero backlinks; do not reactivate without fixing the 2 logged defects — full history in `../pr-backlink-monitor/STATUS.md`, its own tracked project). All TAG Slack alerts lead with `🤖 *The Automations Guide*` (keep for any new alerting workflow).
...
## Current strategic state (2026-08-21)

**Affiliate wave landed; off-site authority still the constraint.** PartnerStack approved 6 programs on 8/21 — ActiveCampaign, FullEnrich, Surfer, KrispCall, Close, Nutshell — all registered live with deep-link variants (55 live registry entries = **32 live programs** + 23 page-specific deep links; every new link curl-verified carrying `ps_partner_key`) and all 6 hubs flipped `listed:true`. Kit/Instantly/Apollo gained deep links too (mains untouched — attribution proven on the existing URLs). First PartnerStack-side click data: Kit 56, Instantly 49, Apollo 36, Lusha 25, lemlist 21, Reply.io 10. **PartnerStack Network app submitted 8/21** with the honest-numbers block (prior rejections traced to a 25x-inflated traffic claim); Pipedrive + Cognism applications remain gated on it. Still awaiting: leadmagic, justcall, lindy, getresponse, adcreative, brevo. Nothing here changes the 8/20 read: the trajectory lever is pitching the pricing index. See [TODO.md](TODO.md).

## Prior strategic state (2026-08-20)

**Technical debt cleared; the constraint is now purely off-site.** Shipped 8/19-8/20: engine priority sort fixed (was publishing Low-first, all 17 Highs starved since June — verified in prod, PR #247 = the oldest High), sitemap `lastmod` + GSC re-read (Google had not read it in 58 days; now 217/217, guarded by the weekly `seo-freshness` GHA), site-wide horizontal scroll below 1280px fixed, 28 affiliate programs live (was 15), 6 new tool hubs. **Index 196/217 (90%), up +17.** A full instrument sweep on 8/20 found **nothing else broken**: zero dead affiliate links, `affiliate_click` beacon firing, Rewardful attribution confirmed on a live click, zero n8n execution errors, all GHAs green, RSS healthy (107 items). **Bing Webmaster Tools IS registered** (2026-04-24, GSC-imported so no on-site artifact exists — do not re-doubt this; see OFF_SITE_SEO_CHECKLIST). **The one real problem left is authority: zero independent referring domains** (measured 8/19 — all 27 linking pages are Ian's own homegrowngrowth.co plus self-created Crunchbase/PH/LinkedIn profiles, and the CC BY pricing index has zero citations anywhere). Publishing stays HOLD at 1/day; weekly clicks 3→4→5→7 against a 10-click gate. **Pitching the pricing index is the only open action that changes the trajectory** (verified target list + template in OFF_SITE_SEO_CHECKLIST.md). See [TODO.md](TODO.md).

## Prior strategic state (2026-08-19)

**The engine's priority sort was inverted from day one and is now fixed (`6802060`).** Notion sorts a select by option order (High, Medium, Low), so `Priority: descending` in Get Next Topic read Low first; the queue drained lowest-priority-first (five Lows 8/10-8/14, then Mediums) while all 17 Highs — the migration formats that actually earn clicks — sat queued since June. Now `ascending` (GET-verified live), so the next ~3 weeks publish the High backlog. 28d GSC to 8/16: **19 clicks / 13,182 impr / pos 37.6**, weekly clicks 3→4→5→7 — rising, still under the 10-click-week gate, **publishing stays HOLD at 1/day** (re-check ~9/09 after the Highs have run). Affiliate reality check: 2 `affiliate_click` events in 28d, so coverage is not the bottleneck, but all 13 tools in the upcoming High queue are `pending` — application batch re-scoped to them in TODO.md (@ian).

## Prior strategic state (2026-08-13)

**The authority sprint's linkable asset is BUILT and live: [/revops-automation-pricing/](src/pages/revops-automation-pricing.astro)** (PR #239 `3c87cec`), 65 of 93 tools priced from their own vendor pages, dataset published under CC BY 4.0. **Pitching it to 5-10 RevOps newsletters is the half that earns links and is still open (@ian).** It was deliberately NOT built on the "waterfall-enrichment hit-rate data" the 8/04 audit suggested: that data does not exist, the twins carry engine-generated "in my testing" prose with no measurement behind it. Ian's call is that those posts stay as-is, so **do not cite them as data.** The index's rule is that the extractor may never use prior knowledge; a figure absent from the fetched page is null with a reason, and a validator nulled 3 invented prices on the live run. **Analytics now read true (PR #238 `2398638`):** PostHog 408442 is shared with FlyrAI, which is 63.5% of all events, and no insight filtered by host, so the dashboard had been overstating pageviews 3.6x and form submits 79x. Insights are host-scoped, deploy previews no longer tracked, `affiliate_click` now carries `$host`. **The project split is DONE (2026-08-13), and TAG's side of it was never in doubt: 408442 is TAG's alone.** FlyrAI's side was mis-recorded twice, corrected 2026-08-17: it did NOT ingest into 516881 ("FlyrAI") as this file claimed. Its traffic landed in **555096 ("Flyr")**, which was NOT the empty decoy an earlier version of this paragraph called it - measured, 555096 held real `flyrai.vercel.app` `$pageview` events while 516881 held 3. **555096 was deleted 2026-08-17** (its 108 events were Ian's own testing), which also removes the "Flyr"/"FlyrAI" name collision behind both wrong records. FlyrAI prod points at 516881 from 2026-08-17 onward. TAG's 408442 was never involved in any of this. The recurring root cause is the same each time: a `phc_` token does not expose its project through any public endpoint, so the id keeps getting INFERRED rather than measured. **The only reliable check is to match the bundle token against `api_token` from `GET /api/projects/<id>/` with a key scoped to that project.** Pre-split history stays in 408442, so `$host` scoping is permanent, not transitional. Two traps fixed with it: filter on BOTH `theautomationsguide.com` and the root-dotted `theautomationsguide.com.`, and never resolve `@current` (it follows whichever project was last opened in the UI, so `posthog-setup.mjs` pins 408442). Known open bug: **the whole site scrolls horizontally at 768px** (header nav, not any one page). GSC unchanged from S75: 16 clicks / 11,423 impr / pos 36.6, publishing stays HOLD at 1/day. See [TODO.md](TODO.md).

## Prior strategic state (2026-08-12)

...
Homepage/tools/blog redesign + analytics + the deferred SVG recolor, all on the SAME pre-merge branch **`design-refresh-brand-kit-v2`** (continues Session 14's work, still unmerged). Five areas:
...
**Revert path:** all changes are markup/CSS/asset/content + one analytics component + CSP + two docs (no schema/route/workflow changes beyond the +1 Kit tool hub). Safe to revert as a unit. After merge: `git revert -m 1 <merge-sha>`. The SVG recolor and the structural changes are in the same working tree this session (not yet committed) — if committed separately, a color-only revert is possible.

...
**The authority sprint's linkable asset is BUILT and live: [/revops-automation-pricing/](src/pages/revops-automation-pricing.astro)** (PR #239 `3c87cec`), 65 of 93 tools priced from their own vendor pages, dataset published under CC BY 4.0. **Pitching it to 5-10 RevOps newsletters is the half that earns links and is still open (@ian).** It was deliberately NOT built on the "waterfall-enrichment hit-rate data" the 8/04 audit suggested: that data does not exist, the twins carry engine-generated "in my testing" prose with no measurement behind it. Ian's call is that those posts stay as-is, so **do not cite them as data.** The index's rule is that the extractor may never use prior knowledge; a figure absent from the fetched page is null with a reason, and a validator nulled 3 invented prices on the live run. **Analytics now read true (PR #238 `2398638`):** PostHog 408442 is shared with FlyrAI, which is 63.5% of all events, and no insight filtered by host, so the dashboard had been overstating pageviews 3.6x and form submits 79x. Insights are host-scoped, deploy previews no longer tracked, `affiliate_click` now carries `$host`. **The project split is DONE (2026-08-13):** FlyrAI now ingests into its own project **516881 ("flyrai")**, verified by the token in the live bundle, by a real browser hitting flyrai.vercel.app, and finally by Ian seeing the events land. A separate empty project 555096 ("Flyr") also exists and receives nothing; an earlier version of this file wrongly named 555096 as FlyrAI's, because a `phc_` token does not expose its project through any public endpoint and the id was INFERRED from `@current` rather than measured. Pre-split history stays in 408442, so `$host` scoping is permanent, not transitional. Two traps fixed with it: filter on BOTH `theautomationsguide.com` and the root-dotted `theautomationsguide.com.`, and never resolve `@current` (it follows whichever project was last opened in the UI, so `posthog-setup.mjs` pins 408442). Known open bug: **the whole site scrolls horizontally at 768px** (header nav, not any one page). GSC unchanged from S75: 16 clicks / 11,423 impr / pos 36.6, publishing stays HOLD at 1/day. See [TODO.md](TODO.md).

...
- [ ] Apply to the 10 Wave-2 programs below (paste from each block): ~3-5 min each, ~40 min total
- [ ] After approval, paste the real link into `src/data/affiliate-links.ts` (`url`) + flip `status` to `live`, then set `listed: true` in `src/data/tools.ts` and add a logo. See [AFFILIATE_PIPELINE.md](AFFILIATE_PIPELINE.md) "How to use this."
- [ ] Mark Applied / Approved in each block below as you go (mirrors the pipeline table)

---

## Standard answers (paste into every application)

| Field | Value |
|---|---|
| Site / website URL | https://theautomationsguide.com |
| Site name | The Automations Guide |
| Owner / contact | Ian Chamberland, ian@theautomationsguide.com |
...
**Audience description (long form, paste verbatim):**

> The Automations Guide is a RevOps and GTM automation publication for go-to-market teams that want to scale output, not headcount. Our readers are founders, revenue operations operators, and sales/marketing ops practitioners actively choosing and wiring up their tech stack: CRM, cold email, enrichment, AI agents, dialers, and the automation middleware (Make, n8n, Zapier) that connects them. Content is neutral, hands-on comparison and how-to writing from operators who have run these tools in production.

**"How will you promote our product?" (paste verbatim, trim per program):**

> We publish neutral, experience-based comparison and how-to content (36+ posts, with a per-tool hub page at /tools/<tool> that carries the product intro, an FAQ, and links to every article that mentions it). Your tool would be promoted through (1) a dedicated hub page and contextual placement in relevant head-to-head comparison posts, (2) our Beehiiv newsletter (daily + weekly sponsor/feature slots), and (3) LinkedIn and X distribution. We rank tools on fit for the reader, not commission size, which keeps recommendations credible and conversion-quality high.

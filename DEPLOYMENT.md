...
|---|---|---|---|---|
| 2026-06-01 | direct | _pending commit_ | Engine: update `n8n/blog-post-engine.json` for the new light format (DIAGRAM COLORS rule in Generate Draft + Humanize prompts, deterministic `colorFixes` remap in `sanitizeMdx()`) **+ requires n8n re-import**; regenerate `apple-touch-icon.png` (180x180 from brand mark); fill design-refresh merge SHA. | `git revert <sha>` (then re-import prior `blog-post-engine.json` from git into n8n Cloud) |

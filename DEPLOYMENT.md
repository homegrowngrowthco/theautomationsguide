...
**Tip:** always note in [docs/SESSION_LOG.md](docs/SESSION_LOG.md) when you re-import or deploy an n8n workflow — that way the audit trail of "which version is live in n8n Cloud right now" is documented somewhere outside n8n.

---

## Part 5: Finding what deployed recently

This section used to hold a hand-maintained deployments table; it went stale (last row 2026-06-08) and is superseded by:

- `git log origin/master --oneline --merges -15` — the real deployment list
- [docs/SESSION_LOG.md](docs/SESSION_LOG.md) — every session entry records the merge SHAs + the exact revert command for that session's changes
- For n8n engine rollbacks: re-run `n8n/deploy-engine.mjs --apply` against a prior `blog-post-engine.json` from git history (see Part 4)

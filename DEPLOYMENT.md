...
|---|---|---|---|---|
| 2026-06-08 | PR #57 (+ bump `48eebc8`) | `bee15f5` | **Topic backlog builder** (Session 22) — a standing topic-discovery engine SEPARATE from the publishing engine, hosted on **GitHub Actions** (`.github/workflows/topic-backlog.yml`, weekly Sun 06:00 UTC + manual `dry_run` dispatch), NOT n8n. [backlog/build-backlog.mjs](backlog/build-backlog.mjs) parses the ~104-tool universe (tools.ts + AFFILIATE_PIPELINE.md), dedups against published posts + the live Notion Content Calendar (one shared `norm()` guard), runs one Sonnet call to rank net-new topics, and with `--stage` writes them as Notion `Status:Suggested` (NEVER `Queued` — human gate intact). Needs the `NOTION_TOKEN` repo secret (the engine's existing integration; access/Bearer token, db `62f34586-…`, version 2022-06-28). `dry_run:true` CI verified: read 82 live calendar rows + 30 posts, proposed 25, zero writes. Bump `48eebc8` = checkout@v6/setup-node@v6/upload-artifact@v7. | `git revert -m 1 bee15f5` (or just delete `.github/workflows/topic-backlog.yml` to stop it); `git revert 48eebc8` for the action bump. Additive — no engine/site impact. |
| 2026-06-04 | PR #55 (+ live n8n API) | `0c73b8a` | **Proactive QA gate** (Session 21). New deterministic linter [qa/lint-content.mjs](qa/lint-content.mjs) (camelCase SVG attrs, em/en dashes, multi-col inline grid/flex wrappers, unknown `/go`+`/tools` slugs, hallucinated/un-imported components, `<style>` blocks) — wired into `qa-content-pr.yml`, replacing the camelCase-only grep step (hard-fails the PR). Refreshed Vision bot prompt ([qa/qa-pr-review.mjs](qa/qa-pr-review.mjs)): light brand + full 18-component list + squish/gap/tree/oversized-embed flags. Engine `sanitizeMdx()` strips multi-col inline wrappers (`update-engine-squish-guard.mjs`, deployed live). **Closed the root cause**: the QA auto-fixer ([qa/qa-pr-fix.mjs](qa/qa-pr-fix.mjs)) was injecting `<style>` grid wrappers against component-internal misdiagnoses — now forbids all `<style>`/wrapper divs, scoped to MDX-only fixes, and **deterministically sanitizes its own output**. Fixed the TOC `columns:2` tablet lopsidedness. All 28 posts brought to a green baseline (`--fix`: dashes + dead `<style>` band-aids removed). Build clean 66 pages. | `git revert -m 1 0c73b8a`; engine: re-run `deploy-engine.mjs --apply` against `git show 0c73b8a~1:n8n/blog-post-engine.json` |
| 2026-06-04 | direct + live n8n API | `f378472` | **Engine layout-hygiene fix** (PR #51 bugs Ian flagged; that post scrapped/closed). (1) Squished StatRow: model wrapped it in a per-post `<style>.stat-row-3up{grid:repeat(3,1fr)}</style>` + div → 1/3 width; `sanitizeMdx()` now strips ALL post-level `<style>` blocks (components are self-styled; bare div left is harmless). (2) Awkward SideBySide+yes/no DecisionTree: removed that forced "how to choose" skeleton step (rotating COMPARISON FORMAT already covers compare+decide, prefers ChooseIf/IntentTable). + LAYOUT prompt rule + Humanize verify (no `<style>`/grid-width wrapper divs). Deployed live + verified (strips style, rule present, SideBySide-howto gone, 1/1 braces, active). | `git revert f378472` then re-run `deploy-engine.mjs --apply` |

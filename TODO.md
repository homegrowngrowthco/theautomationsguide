...
- [ ] Harden the Vision QA bot prompt (`qa/qa-pr-review.mjs`) so it stops false-flagging the deliberately-stacked components (ChooseIf/StatRow/DecisionTree vertical list) as cramped/illegible — the root cause that marks fine posts `shouldFix:true` (deferred from Session 31; run 27414522866 hallucinated an "illegible flowchart" that doesn't exist on mobile) @med [qa/qa-pr-review.mjs](qa/qa-pr-review.mjs)

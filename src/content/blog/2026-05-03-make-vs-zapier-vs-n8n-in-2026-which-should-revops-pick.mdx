...
<ComparisonTable
  title="Make vs Zapier vs n8n — RevOps in 2026"
  tools={[
    {
      name: "n8n",
      tagline: "Best for technical RevOps + cost-sensitive scale",
      pros: [
        "Self-host for ~$20/mo, no per-op pricing",
        "First-class AI agent + LangChain nodes",
        "Error Trigger workflows for instance-wide alerting",
        "JS Code nodes — drop into custom logic anywhere"
      ],
      cons: [
        "Smaller native app catalog (closing fast)",
        "Self-hosting requires someone who can manage Docker + Postgres"
      ],
    ...
    {
      name: "Make",
      tagline: "Pragmatic mid-market choice",
      pros: [
        "Native error routes (table-stakes for production)",
        "Iterators + aggregators handle real list logic",
        "OpenAI module + HTTP for credible AI workflows",
        "Visual canvas accessible to non-engineers"
      ],
      cons: [
        "Self-host gated to enterprise plans",
        "Cost-per-op still beats Zapier but isn't free at high scale"
      ],
    ...
    },
    {
      ...
      pros: [
        "Largest native app catalog",
        "Fastest path to a 'working' automation",
        "Familiar to non-technical operators"
      ],
      cons: [
        "Per-task pricing tax compounds fast",
        "Error handling is genuinely embarrassing for the price",
        "No real iteration, anemic branching, late + awkward sub-workflows",
        "AI is a checkbox feature, not a capability"
      ],

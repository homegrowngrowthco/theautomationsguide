...
Before wiring a single node, make sure three things are in place: API keys for your sequencer ([Smartlead](/go/smartlead) or Instantly), a Clay table configured with the enrichment waterfall you want (I run Clearbit first, then LinkedIn, then [Apollo](/go/apollo) as the final fallback), and a HubSpot or Pipedrive sandbox to test writes against. The [n8n docs on HTTP Request nodes](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/) are genuinely good. Skim them before you start.
